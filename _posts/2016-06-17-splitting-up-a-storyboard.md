...
**How do you go about splitting up your storyboard into multiple smaller storyboards? And why would you? In this article I’ll tell you why we wanted to do this, what kind of problems we encountered along the way, and how we solved them.**
...
Yes, kinda messy and not necessarily super-manageable. And not very informative either, conceptually. So how do we go about spliting it up?

With XCode 7, we got a new, nice feature - Refactor to storyboard… :

XXXXX

You just select all the scenes you want to extract and then this refactor feature will both create a new storyboard for you, and it will wire up any connections between scenes in the old storyboard and the new storyboard. This, however, doesn’t work if you’re supporting iOS 8 AND you’re using relationship segues (i.e. segues from a UITabBarController):
http://stackoverflow.com/a/33691412/1485715

Which is the case for us, of course. But, at least it gives us a handy shortcut for extracting the scenes into a new storyboard. We just deleted the resulting storyboard references. Also, for non-relationship situations we would be on our own anyway.
...
After **a lot** of trial and error here (I can assure you that I’m sparing you a lot of painful details here), we finally gave in and decided to generate separate instantiator classes for objc and Swift. Win! Or was it?
...
That’s all well and good, but since our Python script parses the storyboard files and extracts the storyboard identifiers and their respective custom class names (if any), we had class names **with** the prefix (objc classes) and class names without the prefix (Swift classes). This enabled us to check for this prefix while generating (so it would match the annotated class name), and when generating Objc code, we added the prefix to the classname, and when generating Swift code, we left it as is. Remember, in the storyboard, the non-prefixed Swift class name was used.
...
To be clear, the hurdles I’ve listed here are probably less than half the hurdles and dead ends we’ve met. You would probably be quite bored if I had listed them all. And frankly, I don’t even remember them all anymore. Which is probably a good thing.
...
Allright, what if we define the view controller as “FINUserAdListViewController” in the storyboard scene? Surely, that **has** to work in Objc?
...
Well, almost. You might have noticed we have an enum for the storyboard identifier. We could, of course, skipped the enum and just used the identifier string directly in the instantiate method. We’re undecided on that for now, as we thought we might use the enums for something else. But we might end up removing the enums at a later point.
